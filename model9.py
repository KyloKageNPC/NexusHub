...
def analyze_and_forecast_enhanced(country, operator, variance_method='hybrid', zero_method='smart_interpolation', mode='sarima'):
    """
    ...
    if len(ts_transformed) > 4:
        if mode == 'xgboost':
            print ("\n📦 Using XGBoost model...\n")
            supervised_df = create_features_for_xgboost(ts_cleaned)
            model, X_test, y_test, y_pred = train_xgboost_model(supervised_df)
            ...
            print("\n🔗 Using Hybrid SARIMA + XGBoost model...\n")
            results_pred, actuals = hybrid_forecast(ts_transformed, auto_model.order, auto_model.seasonal_order, transform_info)


            results = pd.DataFrame({
                'Actual': actuals,
                'Predicted': results_pred
            }, index=actuals.index)
        else:
            print("\n🧠 Using SARIMA model...\n")
...
def create_features_for_xgboost(ts, lags=[1, 2, 4]):
    df = ts.to_frame(name='connections')
    for lag in lags:
        df[f'lag_{lag}'] = df['connections'].shift(lag)
    df['rolling_mean_4'] = df['connections'].rolling(4).mean()
...
def hybrid_forecast(ts_transformed, order, seasonal_order, transform_info):
    sarima_model = SARIMAX(ts_transformed, order=order, seasonal_order=seasonal_order)
    sarima_fit = sarima_model.fit(disp=False)

    # In-sample fitted values (drop NaNs from differencing)
    fitted_vals = sarima_fit.fittedvalues.dropna()

    # Residuals for supervised learning
    residuals = ts_transformed.loc[fitted_vals.index] - fitted_vals
    residual_df = create_features_for_xgboost(residuals)

    model, X_test, y_test, xgb_pred = train_xgboost_model(residual_df)

    # Combine SARIMA + XGBoost residuals
    sarima_forecast = fitted_vals.loc[X_test.index]
    hybrid_pred = sarima_forecast + xgb_pred

    # Inverse transform
    inverse_func = transform_info['inverse_func']
    final_forecast = inverse_func(hybrid_pred)

    return final_forecast, y_test

# Example usage
if __name__ == "__main__":
    #analyze_and_forecast_enhanced('Ghana', 'Glo', 'hybrid', 'smart_interpolation', mode='sarima')
    analyze_and_forecast_enhanced('Ghana', 'Glo', 'hybrid', 'smart_interpolation', mode='xgboost')
    #analyze_and_forecast_enhanced('Ghana', 'Glo', 'hybrid', 'smart_interpolation', mode='hybrid')
