...
def forecast_single_operator(country, operator, forecast_periods=8, variance_method='hybrid', zero_method='smart_interpolation', verbose=False):
    """
...
def forecast_multiple_operators(country, operators, forecast_periods=8, variance_method='hybrid', zero_method='smart_interpolation'):
    ...
    for i, operator in enumerate(operators):
        ...
        result = forecast_single_operator(
            country, operator, forecast_periods, variance_method, zero_method, verbose=True
        )
        ...
        if result is not None:
            ...
            print(f"  Forecast end: {forecast_data.iloc[-1]['Forecast']:,.0f}")
            print(f"  Projected growth: {((forecast_data.iloc[-1]['Forecast'] / hist_data.iloc[-1]) - 1) * 100:.1f}%")
    ...
    for operator, result in results.items():
        ...
        # Forecast
        plt.plot(forecast_idx, forecast_data['Forecast'], 's--', 
    ...
    # Formatting
    plt.title(f'Multi-Operator Subscriber Forecasting: {country}', fontsize=16, fontweight='bold')
    ...
    for operator, result in results.items():
        ...
        if len(hist_data) > 1:
            ...
            forecast_data = result['forecast_results']
            forecast_growth = ((forecast_data.iloc[-1]['Forecast'] / hist_data.iloc[-1]) - 1) * 100
...
if __name__ == "__main__":
    # Example usage for Ghana with multiple operators
